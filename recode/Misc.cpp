...
std::vector<uint8_t>* Misc::makeUnboundPESSegment( const uint8_t* ptr, size_t len, unsigned long long pts, unsigned long long dts, unsigned int m_stream_id ) {
	...
	if ( rc != NULL ) {
		...
		if ( dts != 0xFFFFFFFFFFFFFFFFUL ) {
			...
			rc->push_back( 0x80 | 0x00 | 0x00 | 0x00 | 0x00 | 0x00 | 0x00 );	/* PTS+DTS, no-escr, no-esrate, no-dsm, no-additional, no-crc, no-extension */
			rc->push_back( 0x05 );								/* header-size */
			rc->push_back( 0x20 | ( ( ( pts >> 30 ) & 0x07 ) << 1 ) | 0x01 );
		}
