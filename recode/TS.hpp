...
public:
	...
	friend class TS;
	public:
		/// Query how many packets are present for this PID stream.
		/// \return count.
		unsigned int numPackets();

		/// Get a given TS frame from the PID list.
...
public:
	/// Create a TS representation from the given data.
	...
	/// Find out how many bytes would be needed to store all the TS packets for a PID.
	/// \param[in]	pid		PID
	/// \return length.
	size_t sizePIDStream( unsigned pid );

	/// Add a new TS Packet to the TS representation and append to the stream object.
	/// \param[in]	pkt		TSPacket to store reference to.
	void add( TSPacket* pkt );

	/// Replace all the TS packets for a given PID with a new set.
